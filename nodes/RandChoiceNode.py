# -*- coding: utf-8 -*-
# created at: 2022/9/17 11:37
# author    : Gao Kai
# Email     : gaosimin1@163.com


from . import BaseNode


class RandChoiceNode(BaseNode):

    enabled = True

    gui_param = {
        "group": "Arithmetic"
    }
    has_input = True

    template_dict = {
        "name": None,
        "show-name": False,
        "type": "RandChoiceNode",
        "x": 0,
        "y": 0,
        "var": {
            "randomizer": {
                "type": "MChoiceRandomizer",
                "name": None
            },
            "return": {
                "type": "MNum",
                "name": None
            }
        },
        "in-link": set(),
        "out-link": {
            "Next": None,
            "Done": None
        }
    }

    out_num = len(template_dict["out-link"])
    out_enum = dict(zip(template_dict["out-link"].keys(), range(out_num)))

    def __init__(self, runtime_dict):
        super(RandChoiceNode, self).__init__(runtime_dict)

    def run(self, _record):
        _v = self.runtime["var"]["randomizer"].get_value()
        if _v is None:
            return self.runtime["jump"]["Done"]
        else:
            self.runtime["var"]["return"].set_value(_v)
            return self.runtime["jump"]["Next"]
