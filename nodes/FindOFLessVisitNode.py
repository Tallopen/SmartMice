# -*- coding: utf-8 -*-
# created at: 2021/11/1 14:58
# author    : Gao Kai
# Email     : gaosimin1@163.com


from . import BaseNode


class FindOFLessVisitNode(BaseNode):

    enabled = True

    gui_param = {
        "group": "Arithmetic"
    }
    has_input = True

    template_dict = {
        "name": None,
        "show-name": False,
        "type": "FindOFLessVisitNode",
        "x": 0,                             # position X of the node
        "y": 0,                             # position Y of the node
        "var": {
            "open field": {
                "type": "MOpenField",
                "name": None
            },
            "x": {
                "type": "MNum",
                "name": None
            },
            "y": {
                "type": "MNum",
                "name": None
            }
        },
        "in-link": set(),
        "out-link": {
            "Done": None
        }
    }

    out_num = len(template_dict["out-link"])
    out_enum = dict(zip(template_dict["out-link"].keys(), range(out_num)))

    def __init__(self, runtime_dict: dict):
        super(FindOFLessVisitNode, self).__init__(runtime_dict)

    def run(self, _record):
        _x, _y = self.runtime["var"]["open field"].get_pos_by_less_visit()
        self.runtime["var"]["x"].set_value(_x)
        self.runtime["var"]["y"].set_value(_y)
        return self.runtime["jump"]["Done"]
