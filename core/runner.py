...
class Runner:
    ...
    def _run_func(self, _record):
        ...
        try:
            ...
            self._is_running = False
            _record.log('System Event', 'Error Encountered', f"Error at '{_current_node_name}': ERROR MESSAGE - {_error_message}; Running aborted.")
            _record.save()
            self._interface.run_end.emit()
            return False, f"Error at '{_current_node_name}': ERROR MESSAGE - {_error_message}; Running aborted."
